...
#include "crazypod_corner_masks.h"

#define SCREEN_RADIUS_MAX 32

static lv_obj_t *corner_masks[
    CRAZYPOD_CORNER_MASK_SCREEN_COUNT][4];
static uint8_t corner_pixels[4][
    SCREEN_RADIUS_MAX * SCREEN_RADIUS_MAX * 4]
    CACHEALIGN_AT_LEAST_ATTR(16);
static lv_image_dsc_t corner_descriptors[4];
...
static void rebuild_descriptors(void)
{
    ...
    for(corner = 0; corner < 4; ++corner) {
        ...
        lv_image_dsc_t *descriptor = &corner_descriptors[corner];
        uint8_t *pixels = corner_pixels[corner];
        int center_x = (corner == 0 || corner == 2)
            ? radius - 1 : 0;
...
void crazypod_corner_masks_refresh(void)
{
    ...
    rebuild_descriptors();
    for(screen = 0;
        screen < CRAZYPOD_CORNER_MASK_SCREEN_COUNT;
        ++screen) {
        int corner;
        for(corner = 0; corner < 4; ++corner) {
            lv_obj_t *mask = corner_masks[screen][corner];
            int radius = corner < 2
                ? appearance->screen_top_radius
                : appearance->screen_bottom_radius;
            ...
            lv_image_set_src(mask, &corner_descriptors[corner]);
...
void crazypod_corner_masks_attach(lv_obj_t *screen, int screen_index)
{
    ...
    if(screen == NULL || screen_index < 0 ||
       screen_index >= CRAZYPOD_CORNER_MASK_SCREEN_COUNT)
        return;
    for(corner = 0; corner < 4; ++corner) {
        lv_obj_t *mask = lv_image_create(screen);
        corner_masks[screen_index][corner] = mask;
        lv_obj_remove_flag(mask, LV_OBJ_FLAG_CLICKABLE);
    }
    crazypod_corner_masks_refresh();
}
