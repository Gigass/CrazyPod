...
兼容性修正：8 位命令集的 type 0/1 面板保留 Rockbox Bootloader 原本就会发送的
`0x35` TE 命令，但只有运行时 GPIO 信号验证通过后才使用相位同步。16 位命令集的
type 2/3 面板不启用实验性 FMARK 写入，寄存器 `0x090` 保持原值 `0x0021`。
...
type 0/1 面板会进入探测；只有运行时验证通过后，局部同步路径才使用 GPIO TE。
Rockbox Bootloader 对这两类 8 位面板的原始初始化本身已经发送 `0x35`。type 2/3
使用不同的 16 位寄存器接口，当前 type 1 实机结果不能推广到这两类面板。
