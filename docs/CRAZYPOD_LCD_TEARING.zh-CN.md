...
兼容性修正：TE/FMARK 同步现在严格限制为已实机验证的 type 1 面板。type 0、2、3
面板保留 Rockbox 原始初始化并使用普通局部刷新；尤其 type 2/3 的寄存器 `0x090`
恢复为原值 `0x0021`。未验证面板不得仅凭公开资料启用 TE 寄存器写入。
...
只有 type 1 面板会进入探测；并且只有运行时验证通过后，局部同步路径才使用 GPIO
TE。公开资料主要记录 type 2 面板；当前 type 1 实机验证不能据此推广到其他面板。
