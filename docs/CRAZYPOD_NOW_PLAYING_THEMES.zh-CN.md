...
将真机 CPK 直接复制到 `/MiniApps/`。固件在冷启动阶段以及 USB 数据模式退出后
扫描、校验并发布有效包；打开 Mini Apps 或“自定义 > 主题”时只读取已发布目录，
不再执行扫描，也不再使用安装队列。持久化包索引会跳过文件名、大小、修改时间、
包格式和 ABI 均未变化的 CPK；只有新增或变化的包才会重新打开和校验。固件把选择保存到
`/.crazypod/now-playing-theme.cfg`。
