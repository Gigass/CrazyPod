...
#if defined(IPOD_6G)
    /*
     * Letter-key taps generated by macOS accessibility automation can be
     * shorter than the simulator's button polling interval. Queue the iPod
     * simulator's explicit Select/Menu shortcuts as momentary events.
     */
    if(pressed && (key == SDLK_s || key == SDLK_m) && new_btn != BUTTON_NONE)
    {
        static int last_shortcut_key = SDLK_UNKNOWN;
        static long last_shortcut_tick;
        if(key != last_shortcut_key ||
           TIME_AFTER(current_tick, last_shortcut_tick + HZ / 5))
        {
            if(!button_queue_full())
                button_queue_post(new_btn, 0);
            last_shortcut_key = key;
            last_shortcut_tick = current_tick;
        }
        new_btn = BUTTON_NONE;
    }
#endif
