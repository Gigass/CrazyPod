...
void iap_platform_control(struct IAPContext* iap_ctx, enum IAPPlatformControl control, struct IAPPlatformPendingControl pending) {
    ...
    case IAPPlatformControl_TogglePlayPause:
        may_start_playback = true;
        ret = crazypod_usb_iap_control(
            CRAZYPOD_USB_IAP_TOGGLE_PLAY_PAUSE);
        break;
    case IAPPlatformControl_Play:
        may_start_playback = true;
        if(initial_audio_status != AUDIO_STATUS_PLAY)
            ret = crazypod_usb_iap_control(
                CRAZYPOD_USB_IAP_TOGGLE_PLAY_PAUSE);
        break;
    ...
    case IAPPlatformControl_Stop:
        ret = crazypod_usb_iap_control(CRAZYPOD_USB_IAP_STOP);
        break;
    case IAPPlatformControl_Next:
        ret = crazypod_usb_iap_control(CRAZYPOD_USB_IAP_NEXT);
        break;
    case IAPPlatformControl_Prev:
        ret = crazypod_usb_iap_control(CRAZYPOD_USB_IAP_PREVIOUS);
        break;
    case IAPPlatformControl_VolumeUp:
        crazypod_usb_iap_adjust_volume(1);
        break;
    ...
    if(ret && may_start_playback && initial_audio_status == 0) {
...
IAPBool iap_platform_set_shuffle_setting(struct IAPContext* iap_ctx, uint8_t status) {
    ...
    if(status == IAPIPodStateShuffleSettingState_Tracks)
        ...
        crazypod_usb_iap_set_shuffle(false);
    else
        return iap_false;
#else
